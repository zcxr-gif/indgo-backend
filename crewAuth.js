...
/**
 * Is this VA's crew center open?
 *
 * A crew center is opt-in and only Inflight staff can open one, so the answer
 * is the crewCenterEnabled flag. VAs that predate the flag have no field at
 * all: back then a handle was what made a crew center live, so they are
 * grandfathered on that same rule and keep the state they already had. Mirrors
 * the schema default, but for the lean reads every crew lookup uses (lean
 * bypasses mongoose defaults, so the rule has to exist in both places).
 */
function crewCenterOpen(va) {
    if (!va) return false;
    if (va.crewCenterEnabled === undefined || va.crewCenterEnabled === null) return !!va.slug;
    return va.crewCenterEnabled === true;
}

/**
 * Resolve a crew center handle to its VA.
 *
 * Queries WITHOUT the status/opt-in filters and applies them here on purpose: a
 * caller that only gets `null` cannot tell "no VA answers to this address" from
 * "that VA has no crew center" or "it isn't approved yet", and those need
 * different answers from the login page. Only an approved, opted-in VA is ever
 ...
 * @returns {Promise<{va: object|null, reason: 'unknown'|'not-approved'|'not-open'|null, status?: string}>}
 */
async function lookupCrewVa(handle, select) {
    ...
    const found = await VirtualAirlineAd.find(query)
        .select(`${select} status slug crewCenterEnabled`).limit(10).lean();
    ...
    const approved = pool.filter(v => v.status === 'approved');
    if (!approved.length) return { va: null, reason: 'not-approved', status: pool[0].status };
    const va = approved.find(crewCenterOpen);
    // Approved, but nobody opened a crew center for it. Distinct from a bad
    // address: the VA is real and simply didn't want one.
    if (!va) return { va: null, reason: 'not-open' };
...
module.exports = { registerCrewAuthRoutes, viewForRole, verifyCrewRequest, effectiveCaps, lookupCrewVa, crewCenterOpen, CREW_CAPABILITIES, CREW_CAP_IDS };
